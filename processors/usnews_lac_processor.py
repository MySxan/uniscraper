...
def parse_usnews_lac_txt(filepath):
    ...
    while i < len(lines):
        ...
        if (
            line.startswith("#") and "in" in lines[i + 1].strip()
            if i + 1 < len(lines)
            else False
        ):
            # Extract rank number
            ...
            if name and location:
                ...
                records.append({"Rank": rank, "Name": name, "Region": region})
...
def main():
    ...
    try:
        ...
        output_file = (
            Path(__file__).parent.parent / "output" / "USnews_LAC_Rankings.csv"
        )
